...
<body>
    ...
    def generate(self, items_data=None, timestamp=None):
        ...
        # Insert deterministic Trust Header block (minimal v1)
        rendered_at = datetime.utcnow().isoformat(timespec="seconds") + "Z"
        snapshot_id = os.environ.get('PRESENTER_SNAPSHOT_ID', 'UNKNOWN')
        # Derive items_processed: if items_data is a list use its length; if dict with 'items' list use its length; else default to 1
        items_processed = 1
