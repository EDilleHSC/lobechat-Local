...
(async function main(){
  try {
    console.log('approval_invalid_status starting...');
    ...
    // Call /approval with invalid status value
    const payload = { approvedBy: 'Test User', date: new Date().toISOString(), role: 'QA', notes: 'Testing invalid status', checklist: { layout: true, accessibility: true, bugFixed: true, production: true }, status: 'not-a-valid-status' };
    const res = await new Promise((resolve,reject)=>{ const req = http.request({ hostname:'localhost', port: PORT, path: '/approval', method: 'POST', headers: { 'content-type':'application/json', 'x-mcp-approval-token': 'TEST_APPROVAL' }, timeout: 3000 }, (resp)=>{ let b=''; resp.on('data', c=> b+=c); resp.on('end', ()=> resolve({ status: resp.statusCode, body: b })); }); req.on('error', reject); req.write(JSON.stringify(payload)); req.end(); });
    if (res.status !== 400) throw new Error('expected 400 for invalid status enum, got ' + res.status + ' ' + res.body);
    if (res.body.indexOf('status') === -1 && res.body.indexOf('enum') === -1) throw new Error('expected validation details about status in response body: ' + res.body);
    console.log('approval_invalid_status PASSED');
    ...
    process.exit(0);
  } catch (err) { console.error('approval_invalid_status FAILED:', err && err.message ? err.message : err); process.exit(2); }
})();